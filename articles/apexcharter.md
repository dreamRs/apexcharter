...
The objective of this vignette is to show how to quickly build data
visualizations with the ApexCharts JavaScript library, as well as to
give an overview of the different graphics available.
...
Simple line charts can be created with (works with `character`, `Date`
or `POSIXct`):
...
Simple radial charts can be created with (here we pass values directly
