...
Here some advanced configuration examples to use the full potential of
[ApexCharts](https://apexcharts.com/).

## Bar chart

This example is taken from
[{hrbrthemes}](https://github.com/hrbrmstr/hrbrthemes) readme, it use
`mpg` dataset from [{ggplot2}](https://github.com/tidyverse/ggplot2).
...
Modify the tooltip to display “Percentage” instead of the variable name
“pct” :
...
Dataset used is from the UNHCR (The UN Refugee Agency) and contains data
about UNHCR’s populations of concern summarised by continent of origin.
...
Data are in million, in the y-axis we divide by `1e6` to limit the
number of digits :
...
Add an annotation to the chart to identify the [Great Lakes refugee
crisis](https://en.wikipedia.org/wiki/Great_Lakes_refugee_crisis) in
1994 :
...
Configuration for x-axis, hide the tooltip displayed on the axis itself
(not the main tooltip), display two decimals in the labels (useful when
...
Display vertical grid lines (on the x-axis, those on the y-axis are
enabled by default) :
...
Add a custom tooltip with an HTML string, data used in aesthetic can be
accessed in JavaScript with
`w.config.series[seriesIndex].data[dataPointIndex].x` (x is the variable
`gdpPercap`), or
`w.config.series[seriesIndex].data[dataPointIndex].label` (the country),
it’s possible to use custom aesthetics to include more data in the chart
...
This is an adaption of this fancy [{highcharter}
example](http://jkunst.com/highcharter/showcase.md), based from this
[WSJ
...
That’s not possible to make a continuous scale in the legend (like with
...
Missing values are colored by default, above we set them to be displayed
